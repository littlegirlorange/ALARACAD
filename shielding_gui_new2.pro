			...
			if (*pInfo).displayMode eq 'gray-all' then begin
				...
				if nROIs gt 0 then begin
					...
					for iR=0, nRs-1 do begin
						name = rData[eR.name, iR]
						occ  = rData[eR.occ, iR]
						oROI = (*pInfo).oRROIGroup->getByName( name )
...
pro shielding_guiAttachROIs, $
	...
	oPRois = (*pInfo).oPRoiGroup->get( /ALL, COUNT=nPRois )
	oSRois = (*pInfo).oSRoiGroup->get( /ALL, COUNT=n
	oHRois = (*pInfo).oHRoiGroup->count()
	oRRois = (*pInfo).oRRoiGroup->count()
	oQRois = (*pInfo).oQRoiGroup->count()
	oAllRois = nRois + nPRois + nSRois + nHRois + nRRois + nQRois
	if nAllRois eq 0 then return
...
pro shielding_guiAddProtocol, $
	...
	for i=0, nSources-1 do begin
		...
		; Find where to insert this source spec
		widget_control, (*pInfo).wSourceTable, GET_VALUE=table
		nCols = (size( table, /DIMENSIONS ))[0]
		names = table[0,*]
		iRows = where( names eq name, nRows )
...
;			UVALUE='Copy ROI' )
	...
	oPRoiGroup		= obj_new( 'IDLgrModel' )
	oSRoiGroup		= obj_new( 'IDLgrModel' )
	oHRoiGroup		= obj_new( 'IDLgrModel' )
	oQRoiGroup		= obj_new( 'IDLgrModel' )
	...
	oModel->add, oDosemap
	oModel->add, oPRoiGroup
	oModel->add, oSRoiGroup
	oModel->add, oHRoiGroup
	oModel->add, oQRoiGroup
	oModel->add, oXAxis
	oModel->add, oYAxis
...
;			 wInsertHShieldButton:	wInsertHShieldButton, $
			 ...
			 oPRoiGroup:			oPRoiGroup, $
			 oSRoiGroup:			oSRoiGroup, $
			 oQRoiGroup:			oQRoiGroup, $
			 oHRoiGroup:			oHRoiGroup, $
			 oCurROI:				obj_new(), $
			 oFill:					oFill, $
