...
pro shielding_guiCalculate, $
	...
	QNAME=qName, $
	QUIET=bQuiet, $
	Q_SHIELDED_DOSES = qpShieldedDoses, $
	Q_UNSHIELDED_DOSES = qpUnshieldedDoses, $
	Q_LEAD = qpTotalLead, $
	Q_CONC = qpTotalConc, $
	Q_DISTANCES = qpDistances
	...
	bQuery = 1b
	if n_elements( qPoint ) eq 0 then bQuery = 0b

	if n_elements( bQuiet ) eq 0 then bQuiet = 0b
	...
	qpShieldedDoses = fltarr( nPs )
	qpUnshieldedDoses = fltarr( nPs )
	qpTotalLead	= fltarr( nPs )
	qpTotalConc	= fltarr( nPs )
	qpDistances = fltarr( nPs )
	...
	for y=0, nPy-1 do begin
		for x=0, nPx-1 do begin
			...
			if bQuery eq 0b then begin
				...
				curPixel++
				qData = format_float( ([x,y]+0.5)*dmScale, /FLOAT )
			endif else begin
			...
			for iP=0, nPs-1 do begin
				...
				pData *= fpScale
				pData = format_float( pData, /FLOAT )
				...
				if nPRows gt 0 then begin
					...
					pqDist = format_float( pqDist, /FLOAT )

					if bQuery then qpDistances[iP] = pqDist
						...
						for iS=0, nSs-1 do begin
							...
							if retVal ne 0 then begin ; Lines intersect
								...
								if bQuery then begin
									if bIntersects[row] then begin
										case sTable[eS.material,row] of
										'Lead': qpTotalLead[iP]     += float(sTable[eS.thickness,row])
										'Concrete': qpTotalConc[iP] += float(sTable[eS.thickness,row])
										else:
										endcase
									endif
								endif
					...
					if (nHShields gt 0) and (levelIndex gt 0) then begin
						...
						for iH=0, nHShields-1 do begin
							...
							if bQuery then begin
								if bIntersects[iH] then begin
									case hTable[eH.material,iH] of
									'Lead':     qpTotalLead[iP] += float(hTable[eH.thickness,iH])
									'Concrete': qpTotalConc[iP] += float(hTable[eH.thickness,iH])
									else:
									endcase
								endif
							endif
					...
					if bQuery then begin
						if levelIndex gt 0 then qpTotalConc[iP] += dFloor
					endif
					...
					for iPRow=0, nPRows-1 do begin
						...
						if tracerName ne 'I-131 - MIBG' then begin
							; Half-life (h)
							HL = tracer.hl
						...
						for iEnergy=0, nEnergies-1 do begin
							...
							if modality eq 'PET' then begin
								...
								if tracerName ne 'I-131 - MIBG' then begin
									...
									; We assume the user has calculated the cumulated dose
									; and entered it as A0 (in GBq.h instead of GBq).
									doses[iEnergy] = 4.58939 * energy * frac * muEn $
											* NA * A0 * (1.0-PV/100.0) $
										 	* (1.0-SS/100.0) / (pqDist^2)
								endelse
							endelse
								...
								if bQuery then begin
									...
									if modality eq 'SPECT' then begin
										text = '        AF = ' + strtrim(AFTable[eIndex,sIndex],2) $
											 ...
											 + ', TF = ' + strtrim(TFTable[eIndex,sIndex],2)
						 				widget_control, (*pInfo).wQueryText, SET_VALUE=text, /APPEND
						 			endif else begin
						...
						if bQuery then begin
							qpUnshieldedDoses[iP] += total(doses)
							qpShieldedDoses[iP] += total(shieldedDoses)
						endif
				...
				if bQuery then begin
					text = '  ======' + string(15b)
					text += '  ' + pName + ' total unshielded dose = ' $
						 + strtrim(qpUnshieldedDoses[iP],2) + ' uSv' + string(15b)
					text += '  ' + pName + ' total lead            = ' + strtrim(qpTotalLead[iP],2) + ' mm' + string(15b)
					text += '  ' + pName + ' total concrete        = ' + strtrim(qpTotalConc[iP],2) + ' cm' + string(15b)
					text += '  ' + pName + ' total shielded dose   = ' $
						 + strtrim(qpShieldedDoses[iP],2) + ' uSv' + string(15b)
					widget_control, (*pInfo).wQueryText, SET_VALUE=text, /APPEND
				endif
