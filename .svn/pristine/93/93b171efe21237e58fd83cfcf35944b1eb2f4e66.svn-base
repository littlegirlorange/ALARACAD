		...
		if ( n_elements( *(*pInfo).pSeries1 )  gt 1 ) and $
		   ( total( *(*pInfo).pSeries1 ) gt 0 ) then begin
			...
			if (*pInfo).displayMode eq 'gray-all' then begin
				...
				cutoff = alog(50)/alog(1000000)*(*pInfo).topClr
...
;				blendMask = bytarr( (*pInfo).nPx, (*pInfo).nPy )
;				indices = where( img gt 0, nIndices )
;				if nIndices gt 0 then $
;					blendMask[indices] = 1B
				...
				if nROIs gt 0 then begin
					...
					for iR=0, nRs-1 do begin
						...
						if obj_valid( oROI ) then begin
							...
							rOFImg = rMask * occ
							rShieldMask = ( rMask * rOFImg * doseImg ) gt 50
		...
		'P' : begin
			...
			for i=0, nRows-1 do begin
				widget_control, (*pInfo).wSourceTable, $
						...
						SET_VALUE=strtrim([data[0],data[1]],2)
			endfor
		end ; 'P'
		'S' : begin
			eS = (*pInfo).eS
			...
			for i=0, nRows-1 do begin
				widget_control, (*pInfo).wShieldTable, $
						...
						SET_VALUE=strtrim([data[0],data[1],data[3],data[4]],2)
			endfor
		end ; 'S'
		'H' : begin
			eH = (*pInfo).eH
			...
			for i=0, nRows-1 do begin
				widget_control, (*pInfo).wHShieldTable, $
						...
						SET_VALUE=strtrim([data[0],data[1],data[3],data[4], $
										   data[6],data[7],data[9],data[10]],2)
			endfor
		end ; 'H'
		'R' : begin
			eR = (*pInfo).eR
...
pro shielding_guiPrintLength, pInfo, oROI
	...
	void = dialog_message( /INFO, TITLE=name, strtrim( length, 2 ) + ' m' )
...
pro shielding_guiSetROIColours, $
	...
	for i=0, nROIs-1 do begin
		...
		case prefix of
			...
			'H': begin
				rows = where( hTable[eH.names,*] eq name, nRows )
				if nRows eq 1 then begin
...
@'shielding_gui_shield_specs'
	...
	if err_status ne 0 then return, bValid = 0b
...
@'shielding_gui_shield_specs'
	...
	; Columns 1, 3, 5, 7: xcoords
		...
		if ( 0 le coord ) and ( coord le (*pInfo).nPx * scale ) then begin
			...
			shielding_guiUpdateRectROI, pInfo, sEvent
		endif else begin
			bValid = 0b
		endelse
...
pro shielding_guiUpdateImageInfo, $
	...
	if keyword_set( coord ) then begin
		...
		if bEventInImage then begin
			...
			mCoord = $
					'x:' + strtrim( string( coord[0]*fpScale, FORMAT='(f8.2)' ), 2 ) + 'm, ' + $
					'y:' + strtrim( string( coord[1]*fpScale, FORMAT='(f8.2)' ), 2 ) + 'm, '
			...
			dose = $
					'dose:' + strtrim( string( (*(*pInfo).pSeries1)[coord[0],coord[1]], $
							FORMAT='(f10.2)' ), 2 ) + 'uSv'
	...
	widget_control, (*pInfo).wPixelInfoText, $
			SET_VALUE=(mCoord + pixCoord + dose)
...
pro shielding_guiSetHShieldSpecs, $
	...
	widget_control, (*pInfo).wHShieldTable, $
			USE_TABLE_SELECT=[0,iRow,nCols-1,iRow], $
			SET_VALUE=[name, $
			strtrim([data[0],data[1],data[3],data[4],$
			data[6],data[7],data[9],data[10]],2), $
			h, material, thickness, desc], $
			SET_UVALUE=++curRow
...
pro shielding_guiAddSourceSpecs, $
	...
	widget_control, (*pInfo).wSourceTable, $
			USE_TABLE_SELECT=[0,iRow,nCols-1,iRow], $
			SET_VALUE=[name, $
					   strtrim(data[0]*scale, 2), $
					   strtrim(data[1]*scale, 2), $
					   tName, a0, tu, ti, na, pv, ss, desc]
	widget_control, (*pInfo).wSourceTable, $
			SET_UVALUE=++curRow
...
pro shielding_guiHideROIs, $
	...
	for i=0, nROIs-1 do begin
		...
		oROIs[i]->getProperty, NAME=name, UVALUE=oChildROI
		prefix = (strsplit( name, '_', /EXTRACT ))[0]
		if prefix eq type then begin
			oROIs[i]->setProperty, HIDE=hide, TEXT_HIDE=hide
			if obj_valid( oChildROI ) then begin
				oChildROI->setProperty, HIDE=hide, TEXT_HIDE=hide
			endif
		endif
...
pro shielding_guiSetScale, $
	...
	if n_elements( scale ) eq 0 then begin
		...
		if not bCancel then begin
			...
			scale = strtrim( length/pixLength, 2 )
		endif else begin
			return
		endelse
	...
	widget_control, (*pInfo).wFloorplanScaleText, SET_VALUE=scale
...
pro shielding_guiFindMax, $
	...
	widget_control, (*pInfo).wFloorplanScaleText, GET_VALUE=value
	fpScale = float( value[0] )
	maxCoords = array_indices( mask, maxPixel ); + 0.5*dmScale/fpScale
	...
	for i=0, nRows-1 do begin
		widget_control, (*pInfo).wRegionTable, $
				...
				SET_VALUE=[strtrim( maxCoords*fpScale, 2 ), $
						   strtrim( maxValue, 2 )]
	...
	oMaxROI = obj_new( 'IDLgrROI', NAME=name, DATA=maxCoords, STYLE=0, $
			SYMBOL=oSymbol, COLOR=[0,0,255], HIDE=hide )
	oROIGroup->add, oMaxROI

	oDispMaxROI = obj_new( 'MKgrROI', NAME=name, DATA=maxCoords, STYLE=0, $
			SYMBOL=oSymbol, COLOR=[0,0,255], HIDE=hide, $
...
function shielding_guiLinesIntersect, $
	...
	if den eq 0 then begin
		...
		A = numA/den
		B = numB/den
		if ( 0 lt A ) and ( A lt 1 ) and $
		   ( 0 lt B ) and ( B lt 1 ) then begin
	...
	case (*pInfo).mode of
		...
		'move_roi': begin
			widget_control, (*pInfo).wMoveROIButton, $
					SET_VALUE='bitmaps\hand.bmp', /BITMAP, $
					SET_BUTTON=0
...
pro shielding_guiKeepLastMode, $
	...
	case (*pInfo).mode of
		...
		'move_roi': begin
			widget_control, (*pInfo).wMoveROIButton, $
					SET_VALUE='bitmaps\hand_active.bmp', /BITMAP, $
					SET_BUTTON=1
		end ; move_roi
...
;
function shielding_guiRenameROI, pInfo, NAME=namePn

	oROI = (*pInfo).oCurROI
	oDispROI = (*pInfo).oDispCurROI
	if not obj_valid( oROI ) then return, 0

	oROIGroup = (*pInfo).oROIGroup
	oROIs = oROIGroup->get( /ALL, COUNT=nROIs )

	; Make sure this is a unique name for this slice
	bDuplicate = 0b
	if nROIs gt 0 then begin
		for i=0, nROIs-1 do begin
			if oROIs[i] ne oROI then begin
				oROIs[i]->getProperty, NAME=name
				if strupcase( strtrim(name,2) ) eq strupcase( namePn ) then $
					bDuplicate = 1b
			endif
        endfor
	endif
	if bDuplicate then return, 0

	; Rename the ROIs and redisplay
	oROI->setProperty, NAME=namePn
	oDispROI->setProperty, NAME=namePn
	(*pInfo).oWindow->draw, (*pInfo).oViewGroup
...
;--------------------------------------------------------------------
;
;    PURPOSE  Start drawing a new poly ROI
;
pro shielding_guiPolyROIStart, $
	pInfo, $
	imgCoords
	...
	if obj_valid( oROI ) eq 0 then begin
		...
		; Create new display ROI
		oDispROI = obj_new( 'MKgrROI', COLOR=[0,255,0], STYLE=1, $
				THICK=(*pInfo).roiThick, TEXT_SIZE=(*pInfo).textSize )
		(*pInfo).oModel->add, oDispROI
		(*pInfo).oDispCurROI = oDispROI
...
end ; of shielding_guiPolyROIStart


;--------------------------------------------------------------------
;
;    PURPOSE  Complete the new poly ROI
;
pro shielding_guiPolyROIStop, pInfo
	...
	; Make sure the ROI has at least 3 vertices
	...
	if n_elements( roiData )/3 ge 3 then begin
		...
		oROIGroup = (*pInfo).oROIGroup
		oROI->getProperty, DATA=data
		name = shielding_guiGenerateROIName( oROIGroup )
		oROI->setProperty, NAME=name
		oDispROI->setProperty, NAME=name ;, TEXT_ANCHOR=data[*,0]
		...
		oDispROI->setProperty, STYLE=2, COLOR=[255,0,0]

	endif else begin

		; Less than 3 vertices -> delete and remove reference
		; in state
		shielding_guiAbandonROI, pInfo
...
end ; of shielding_guiPolyROIStop


;--------------------------------------------------------------------
;
;    PURPOSE  Start drawing a new freehand ROI
;
pro shielding_guiFreeROIStart, $
	pInfo, $
	imgCoords
...
end ; of shielding_guiFreeROIStart


;--------------------------------------------------------------------
;
;    PURPOSE  Build a freehand ROI
;
pro shielding_guiFreeROIMotion, pInfo, sEvent
	...
	; Add to the display and redraw
	oDispROI->appendData, [imgCoords[0], imgCoords[1]]
	(*pInfo).oWindow->draw, (*pInfo).oViewGroup

end ; of shielding_guiFreeROIMotion


;--------------------------------------------------------------------
;
;    PURPOSE  Complete the new freehand ROI
;
pro shielding_guiFreeROIStop, pInfo
	...
	if n_elements( roiData )/3 ge 3 then begin
		...
		name = shielding_guiGenerateROIName( oROIGroup )
		oROI->setProperty, NAME=name
		for iVp=0, (*pInfo).nVps-1 do begin
			oDispROIs[iVp]->setProperty, NAME=name
		endfor
...
end ; of shielding_guiFreeROIStop
...
;
pro shielding_guiAddSourcePoint, $
	pInfo, $
	imgCoords
...
pro shielding_guiAddQueryPoint, $
	...
	if obj_valid( oROI ) eq 0 then begin
		...
		; Prompt user to set specification and add to display table
		case (*pInfo).level of

			'above': $
				shielding_guiSetQueryDataAbove, pInfo, oROI, /PROMPT
			'below': $
				shielding_guiSetQueryDataBelow, pInfo, oROI, /PROMPT
			else: $
...
;--------------------------------------------------------------------
;
;    PURPOSE  Get coordinates of the nearest S ROI vertex
;
...
;			  graphical ROI
;
function shielding_guiSetCurrentROI, pInfo, oDispROI, NONE=none
	...
	if obj_valid( oDispROI ) eq 0 then return, 0
...
pro shielding_guiEvent, $
	...
	case sEvent.id of
		...
		(*pInfo).wPolyROIButton : begin
			...
			; Update info text
			shielding_guiUpdateInfoText, pInfo, $
					VALUE='Left click to mark the vertices of the polygonal ROI. ' + $
					'Right click to close the ROI.'

			; Wait for the user to start drawing

		end

		(*pInfo).wFreeROIButton : begin
			...
			; Update info text
			shielding_guiUpdateInfoText, pInfo, $
					VALUE='Left click and drag to draw a freehand ROI. ' + $
					'Right click to close the ROI.'
		...
		(*pInfo).wHideSourcesButton: begin
			...
			widget_control, (*pInfo).wHideSourcesButton, GET_VALUE=value

			if value eq 'Hide' then begin

				; Change the button to 'Show'
				widget_control, (*pInfo).wHideSourcesButton, $
						SET_VALUE='Show', $
						SET_BUTTON=1, $
						TOOLTIP='show all source points'

				; Hide the ROIs on the image
				shielding_guiHideROIs, pInfo, /SOURCE, HIDE=1
				...
				widget_control, (*pInfo).wHideSourcesButton, $
						SET_VALUE='Hide', $
						SET_BUTTON=0, $
						TOOLTIP='hide all source points'

				; Show the ROIs on the image
				shielding_guiHideROIs, pInfo, /SOURCE, HIDE=0
		...
		(*pInfo).wHideShieldsButton: begin
			...
			widget_control, (*pInfo).wHideShieldsButton, GET_VALUE=value

			if value eq 'Hide' then begin

				; Change the button to 'Show'
				widget_control, (*pInfo).wHideShieldsButton, $
						SET_VALUE='Show', $
						SET_BUTTON=1, $
						TOOLTIP='show all shield ROIs'

				; Hide the ROIs on the image
				shielding_guiHideROIs, pInfo, /SHIELD, HIDE=1
				...
				widget_control, (*pInfo).wHideShieldsButton, $
						SET_VALUE='Hide', $
						SET_BUTTON=0, $
						TOOLTIP='hide all shield ROIs'

				; Show the ROIs on the image
				shielding_guiHideROIs, pInfo, /SHIELD, HIDE=0
		...
		(*pInfo).wHideHShieldsButton: begin
			...
			widget_control, (*pInfo).wHideHShieldsButton, GET_VALUE=value

			if value eq 'Hide' then begin

				; Change the button to 'Show'
				widget_control, (*pInfo).wHideHShieldsButton, $
						SET_VALUE='Show', $
						SET_BUTTON=1, $
						TOOLTIP='show all horizontal shield ROIs'

				; Hide the ROIs on the image
				shielding_guiHideROIs, pInfo, /HSHIELD, HIDE=1
				...
				widget_control, (*pInfo).wHideHShieldsButton, $
						SET_VALUE='Hide', $
						SET_BUTTON=0, $
						TOOLTIP='hide all horizontal shield ROIs'

				; Show the ROIs on the imate
				shielding_guiHideROIs, pInfo, /HSHIELD, HIDE=0
		...
		(*pInfo).wHideRegionsButton: begin
			...
			widget_control, (*pInfo).wHideRegionsButton, GET_VALUE=value

			if value eq 'Hide' then begin

				; Change the button to 'Show'
				widget_control, (*pInfo).wHideRegionsButton, $
						SET_VALUE='Show', $
						SET_BUTTON=1, $
						TOOLTIP='show all region ROIs'

				; Hide the ROIs on the image
				shielding_guiHideROIs, pInfo, /REGION, HIDE=1
				...
				widget_control, (*pInfo).wHideRegionsButton, $
						SET_VALUE='Hide', $
						SET_BUTTON=0, $
						TOOLTIP='hide all region ROIs'

				; Show the ROIs on the image
				shielding_guiHideROIs, pInfo, /REGION, HIDE=0
...
;			shielding_guiSaveSession, pInfo, FILENAME=filename
		...
		(*pInfo).wQueryButton: begin

			; Reset last mode
			shielding_guiResetLastMode, pInfo

			if (*pInfo).mode eq 'query' then begin
				...
				widget_control, (*pInfo).wQueryButton, $
						SET_VALUE='Stop', $
						SET_BUTTON=1, $
						TOOLTIP='stop adding query points'

				shielding_guiUpdateInfoText, pInfo, $
					VALUE='Left click to mark query point. ' + $
					'Press Stop to exit query mode.'
		...
		(*pInfo).wHideQueryButton: begin
			...
			widget_control, (*pInfo).wHideQueryButton, GET_VALUE=value

			if value eq 'Hide' then begin

				; Change the button to 'Show'
				widget_control, (*pInfo).wHideQueryButton, $
						SET_VALUE='Show', $
						SET_BUTTON=1, $
						TOOLTIP='show query points'

				; Hide the query points on the image
				shielding_guiHideROIs, pInfo, /QUERY, HIDE=1
				...
				widget_control, (*pInfo).wHideQueryButton, $
						SET_VALUE='Hide', $
						SET_BUTTON=0, $
						TOOLTIP='hide query points'

				; Show the query points on the image
				shielding_guiHideROIs, pInfo, /QUERY, HIDE=0
			...
			if sEvent.type eq 0 then begin ; Button press
				...
				case (*pInfo).mode of
					...
					'source_roi': begin
						...
						if sEvent.press eq 1 then begin ; Left button press

							imgCoords = shielding_guiScrn2ImgPix( pInfo, $
									[sEvent.x, sEvent.y] )

							shielding_guiAddSourcePoint, pInfo, imgCoords
							bOk = shielding_guiSetCurrentROI( pInfo, /NONE )
							shielding_guiUpdateROIButtons, pInfo
					...
					'poly_roi' : begin
						...
						if sEvent.press eq 1 then begin	; Left button press
							...
							shielding_guiPolyROIStart, pInfo, imgCoords
					...
					'free_roi' : begin
						...
						if sEvent.press eq 1 then begin	; Left button press
							...
							shielding_guiFreeROIStart, pInfo, imgCoords
					...
					'move_roi_vertex' : begin
						...
						if sEvent.press eq 1 then begin ; Left button press
							...
							if obj_valid( oDispROI ) then begin
								...
								; Clear the undo/redo list if we've selected a new ROI
								bSameROI = 0b
								void = where( (*pInfo).oDispCurROI eq oDispROI, bSameROI )
								if not bSameROI then $
									shielding_guiClearUndoRedoROI, pInfo
					...
					'move_roi' : begin
						...
						if sEvent.press eq 1 then begin ; Left button press
							...
							if obj_valid( oDispROI ) then begin
								...
								; Clear the undo/redo list if we've selecte a new ROI
								bSameROI = 0b
								void = where( (*pInfo).oDispCurROI eq oDispROI, bSameROI )
								if not bSameROI then $
									shielding_guiClearUndoRedoROI, pInfo
				...
				case (*pInfo).mode of
					...
					'poly_roi' : begin
						...
						shielding_guiPolyROIStop, pInfo
						shielding_guiClearUndoRedoROI, pInfo
						shielding_guiUpdateROIButtons, pInfo
					...
					'free_roi': begin
						...
						shielding_guiFreeROIStop, pInfo
						shielding_guiClearUndoRedoROI, pInfo
						shielding_guiUpdateROIButtons, pInfo
...
;					shielding_guiUpdateInfoText, pInfo, VALUE=''
                ...
                'File': case uv1[1] of
                    ...
                    'Open': case uv1[2] of
						...
						'Dose image (.img)' : begin
							; Make sure we have image data first
							if n_elements( *(*pInfo).pSeries0 ) gt 1 then begin
								; Load image
								bHaveFile = shielding_guiOpenDoseImage( pInfo )
								shielding_guiUpdateROIButtons, pInfo
							endif else begin
								; Warn and return
								void = dialog_message( 'Must load floorplan first', $
                    ...
                    'Quit': begin
						...
						; before exiting
						answer = dialog_message( 'Save ROIs before exiting?', $
								/QUESTION, TITLE='Quitting application' )
						if answer eq 'Yes' then shielding_guiSaveROIs, pInfo
...
pro shielding_gui, $
	...
	wSubBase = widget_base( wTopBase, ROW=3 )
		...
		wROIDrawBase = widget_base( wButtonBase, /ROW, $
			...
			wBase = widget_base( wROIDrawBase, /EXCLUSIVE, $
					SPACE=0, XPAD=0, YPAD=0 )
			wPolyROIButton = widget_button( wBase, $
					VALUE='bitmaps\segpoly.bmp', /BITMAP, /NO_RELEASE, $
					TOOLTIP='polygonal ROI' )

			wBase = widget_base( wROIDrawBase, /EXCLUSIVE, $
					SPACE=0, XPAD=0, YPAD=0 )
			wFreeROIButton = widget_button( wBase, $
					VALUE='bitmaps\freepoly.bmp', /BITMAP, /NO_RELEASE, $
		...
		wROIEditBase = widget_base( wButtonBase, /ROW, $
			...
			wMoveROIButton = widget_button( wBase, $
					VALUE='bitmaps\hand.bmp', /BITMAP, /NO_RELEASE, $
		...
		wCalcBase = widget_base( wButtonBase, /ROW, $
				SPACE=0, /TOOLBAR )

			wBase = widget_base( wCalcBase, $
					SPACE=0, XPAD=0, YPAD=0 )
			wCalcThicknessButton = widget_button( wBase, $
					VALUE='bitmaps\axis.bmp', /BITMAP, /NO_RELEASE, $
		...
		wLeftBase = widget_base( wWinBase, /COLUMN, /FRAME )
			...
			wDosemapScaleText = widget_text( wBase, VALUE='0.3', XSIZE=10  )
			...
			wQueryButtonBase = widget_base( wQueryBase, /ROW, SPACE=0, /TOOLBAR, $
					/ALIGN_CENTER )
			wQueryButton = widget_button( wQueryButtonBase, VALUE='Add', $
					/NO_RELEASE, TOOLTIP='add query point' )
			...
			wResolutionText = widget_text( wBase, VALUE='0.3', /EDIT ) ; Should be same as wDosemapScaleText to start
			wBase = widget_base( wCalculateBase, /ROW )
			wShieldHeightLabel = widget_label( wBase, VALUE='Default shielding height (m): ' )
			wShieldHeightText = widget_text( wBase, VALUE='2.1336', /EDIT )
			wBase = widget_base( wCalculateBase, /ROW )
			...
			wAboveDistText = widget_text( wBase, VALUE='3.81', /EDIT )
			wBase = widget_base( wCalculateBase, /ROW )
			...
			wBelowDistText = widget_text( wBase, VALUE='3.81', /EDIT )
			wBase = widget_base( wCalculateBase, /ROW )
		...
		wInfoText = widget_text( wInfoBase, $
	...
	wButton = widget_button( wShieldContextMenu, VALUE='Delete', $
			UVALUE='Delete ROI' )
	wButton = widget_button( wShieldContextMenu, VALUE='Add specs', $
			UVALUE='Add specs' )
	...
	wButton = widget_button( wSourceContextMenu, VALUE='Add specs', $
			UVALUE='Add specs' )
	wButton = widget_button( wSourceContextMenu, VALUE='Print distances', $
			UVALUE='Print distances' )
	...
	wCopyMenuButton = widget_button( wRegionContextMenu, VALUE='Copy', $
			UVALUE='Copy ROI' )
    ...
    info = { groupBase:				groupBase, $
			 ...
			 wPolyROIButton:		wPolyROIButton, $
			 wFreeROIButton:		wFreeROIButton, $
