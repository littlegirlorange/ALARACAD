...
;				endif
				...
				; Generate a flooring shielding mask
				widget_control, (*pInfo).wHShieldTable, GET_VALUE=hData, GET_UVALUE=nHs
				mask	= fltarr( (*pInfo).nPx, (*pInfo).nPy )
				img		= fltarr( (*pInfo).nPx, (*pInfo).nPy )
				doseImg = *(*pInfo).pSeries1
				...
				for iH=0, nHs-1 do begin
					...
					oFill = obj_new( 'IDLgrPattern', STYLE=1, ORIENTATION=45 ) ; line fill
					oPolygon = obj_new( 'IDLgrPolygon', FILL_PATTERN=oFill, COLOR=[255,0,0], $
							DATA=data )
					(*pInfo).oModel->add, oPolygon
