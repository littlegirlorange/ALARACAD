...
;           is [0, NCOLORS].
;
;		TEXTPOS:  Sets the tick text above (0) or below (1) the ruler line.
;
...
; SIDE EFFECTS:
;
;       A HRuler structure is created. The colorbar INHERITS IDLgrMODEL.
...
;
;       thisColorBarObject = Obj_New('HRuler')
...
FUNCTION HRuler::INIT, Position=position, Text=text, $
   	Title=title, $
...
IF N_Elements(range) EQ 0 THEN self.range = [0, 1] $
...
textAxis = Obj_New("IDLgrAxis", 0, Color=self.color, $
    Major=self.major, Minor=self.minor, Title=thisTitle, Range=self.range, /Exact, $
    XCoord_Conv=longScale, Location=[1000, self.position(3), 0.001], _Extra=extra)
textAxis->GetProperty, TickText=thisText
thisText->SetProperty, Font=thisFont, Recompute_Dimensions=2

longAxis2 = Obj_New("IDLgrAxis", 0, Color=self.color, /NoText, $
    Major=self.major, Minor=self.minor, Range=self.range, HIDE=1, $
...
PRO HRuler::GetProperty, Position=position, Text=text, $
    Title=title, Major=major, Minor=minor, $
...
PRO HRuler::SetProperty, Position=position, Text=text, $
    Title=title, Major=major, Minor=minor, $
...
IF N_Elements(position) NE 0 THEN BEGIN
	...
	; MK -- have to get and reset tickText for some annoying
	; reason, otherwise it's reset back to default values.
	self.textaxis->getProperty, TICKTEXT=tickText
	tickText->getProperty, STRINGS=strings
    self.textaxis->SetProperty, XCoord_Conv=longScale, $
       Location=[1000, self.position(1), 0.001], $
       TICKTEXT=tickText
    tickText->setProperty, STRINGS=strings
    self.longaxis2->SetProperty, XCoord_Conv=longScale, $
       Location=[1000, self.position(3), 0.001]
...
ruler = { HRULER, $
             ...
             Position:FltArr(4), $       ; The position of the ruler.
