...
pro shielding_guiSaveQueryDoseTable, $
	...
	openw, lun, /GET_LUN, file
	printf, lun, 'QPoint;' + 'Description;' + strjoin('distance from '+pNames,';') + ';' $
			+ strjoin('mm lead between '+pNames,';') + ';' + strjoin('cm conc between ' +pNames,';') + ';' $
			+ 'Shielded Dose (uSv/y);'
	...
	for iQ=0, nQRois-1 do begin
		...
		if not obj_valid( oQRoi ) then continue
		oQRoi->getProperty, DATA=qData
		shielding_guiCalculate, pInfo, QPOINT=qData, QNAME=qNames[iQ], $
				Q_LEAD=lead, Q_CONC=conc, $
				Q_SHIELDED_DOSES=shieldedDoses, Q_DISTANCES=distances

		line = qNames[iQ] + ';' $
			 + qDescs[iQ] + ';' $
			 + strjoin(strtrim(distances,2),';') + ';' $
			 + strjoin(strtrim(lead,2),';') + ';' $
			 + strjoin(strtrim(conc,2),';') + ';' $
			 + strtrim(total(shieldedDoses),2) + ';'
...
pro shielding_guiSaveDoseImage, pInfo
	...
	if file ne '' then begin
		...
		printf, lun, "pixSizeX (m) = ", strtrim( fpScaleText[0], 2 )
		printf, lun, "pixSizeY (m) = ", strtrim( fpScaleText[0], 2 )
		close, lun
