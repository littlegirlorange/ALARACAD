...
; CALLING SEQUENCE:
;
;  thetext = shielding_gui_mode_selector()
;
; INPUTS:
;
;  None.
...
;
;       theText = shielding_gui_mode_selector(Title='Provide Phone Number...', Label='Number:', Cancel=cancelled)
...
;
;          thetext = shielding_gui_mode_selector(Group_Leader=event.top)
;
...
;
;  VALUE: A string variable that is the intial value of the shielding_gui_mode_selector. By default, a null string.
;
...
PRO shielding_gui_mode_selector_CenterTLB, tlb
...
PRO shielding_gui_mode_selector_Event, event
...
FUNCTION shielding_gui_mode_selector, Title=title, $
...
if n_elements(dropValues) eq 0 then dropValues[0] = ''
...
tlb = Widget_Base(Title=title, Column=1, /Modal, $
   /Base_Align_Center, Group_Leader=groupleader, XSIZE=xSize )

   ; Create the rest of the widgets.

wDropBase = widget_base( tlb, /ROW, /ALIGN_CENTER )
...
shielding_gui_mode_selector_CenterTLB, tlb
...
XManager, 'shielding_gui_mode_selector', tlb
