...
;	Version:
;				080918
;				Modified version of shielding_guiCalculate for cyclotrons
;
;	Usage:
;				shielding_guiCalcCyclo, pInfo[, QPOINT=qPoint, QNAME=qName]
;				pInfo  - pointer to main program info structure.
;				qPoint - [opt] query point to calc dose to. If specified
;						 program calcs dose to single point instead of
;						 entire floor plan. Dose image not changed.
;				qName  - [opt] name of query point to calc dose to. Required
;						 if qPoint is specified.
;
;	Outputs:
;				Dose image saved to info structure (*pInfo.pSeries1) if
;				qPoint and qName are not specified.
;
;	Required Modules:
...
;				shielding_gui_CYCLO_target_specs
;
;	Written by:
;				Maggie Kusano, September 18, 2008
;
;==============================================================================

pro shielding_guiCalcCyclo, $
	pInfo, $
	QPOINT=qPoint, $
	QNAME=qName
	...
	if (*pInfo).modality ne 'cyclo' then return
