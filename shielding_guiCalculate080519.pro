;--------------------------------------------------------------------
;
;    PURPOSE  Calculate the annual dose image
;
				...
				if nPRows gt 0 then begin
					...
					for iPRow=0, nPRows-1 do begin
						...
						if count ne 0 then begin
							...
							TU		= float( pTable[eP.TU,pRows[iPRow]] ) ; uptake time (h)
							TI		= float( pTable[eP.TI,pRows[iPRow]] ) ; imaging time (h)
							...
							if levelIndex gt 0 then begin
								...
								for iH=0, nHShields-1 do begin
									...
									p1 = [pData[0], pData[1], 1.0]
									p2 = [qData[0], qData[1], dist+1-0.5]
									...
									if bIntersects then TF *= HTFTable[tIndex,iH]
								endfor
							endif
