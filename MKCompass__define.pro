...
oNorthFont = Obj_New('IDLgrFont', 'Helvetica', Size=fontsize)
oNorthText = Obj_New('IDLgrText', 'N', Color=self.color, $
    Font=oNorthFont, Recompute_Dimensions=2, /Enable_Formatting, $
...
longAxis = obj_new( 'IDLgrAxis', 1, COLOR=self.color, RANGE=[0,1], $
	...
	LOCATION=self.position, _EXTRA=extra )
oNorthText->setProperty, LOCATION=[self.position[0],self.position[1]+0.09]
...
PRO MKCompass::GetProperty, Position=position, NORTH=north, $
...
if arg_present(north) then north = self.north
...
IF N_Elements(position) NE 0 THEN BEGIN
	...
	self.oNorthText->setProperty, LOCATION=[self.position[0],self.position[1]+0.09]
ENDIF
...
IF N_Elements(color) NE 0 THEN BEGIN
    ...
    self.longAxis->setProperty, COLOR=color
    self.oNorthText->setProperty, COLOR=color
ENDIF
...
ruler = { MKCompass, $
             ...
             oNorthFont:Obj_New(), $     ; The annotation font object.
             oNorthText:Obj_New(), $	 ; The annotation text object.
             shortAxis:Obj_New(), $      ;
             longAxis:obj_new(), $
