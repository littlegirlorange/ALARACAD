...
;--------------------------------------------------------------------
;
;    PURPOSE  Open a dosemap
;			  New dosemap must have the same dimensions and scale
;			  as the floorplan.
;
function shielding_guiOpenDosemap, pInfo
	...
	file = dialog_pickfile( FILTER = '*.img', $
							PATH = (*pInfo).workingDir, $
							GET_PATH = inDir, $
							TITLE='Select the dosemap image (*.img)' )

	; Return if the user cancels or selects nothing
	if file eq '' then return, bHaveFile

	fileBase = file_basename( file, '.img' )
	info = file_info( file )
	if info.size/4 ne double((*pInfo).nPx)*(*pInfo).nPy then begin
		void = dialog_message( /ERROR, 'Incorrect dimensions. Image not loaded.' )
		return, 0b
	endif

	image = fltarr( (*pInfo).nPx, (*pInfo).nPy )
	openr, lun, /GET_LUN, file
	readu, lun, image
	close, lun
	*(*pInfo).pSeries1 = image

	return, 1b

end ; of shielding_guiOpenDosemap


;--------------------------------------------------------------------
;
;    PURPOSE  Add a dosemap
;			  Second dosemap must have the same dimensions and scale
;			  as the floorplan.
;
function shielding_guiAddDosemap, pInfo
...
pro shielding_guiAddROIs, $
	...
	for iROI=0, nROIs-1 do begin
		...
		; Assign new name
		oROIs[iROI]->getProperty, NAME=oldName
		name = shielding_guiGenerateROIName( (*pInfo).oROIGroup, PREFIX=prefix )
		oROIs[iROI]->setProperty, NAME=name
		...
		if prefix ne 'Q' then begin
			; Find data in table
			iRows = where( data[0,*] eq oldName, nRows )
			if nRows gt 0 then begin
				widget_control, widget,GET_UVALUE=curRow, GET_VALUE=table
				nCols = (size(table, /DIM))[0]
				for iRow=0, nRows-1 do begin
					entry = data[*,iRows[iRow]]
					entry[0] = name
					widget_control, widget, $
...
;			uVal		- number of rows in the table
;
pro shielding_guiOpenSources, $
	pInfo

	; Prompt for file
	inFile = dialog_pickfile( FILTER = '*_sourceTable.txt', $
							  PATH = (*pInfo).workingDir, $
							  GET_PATH = inDir, $
							  TITLE="Select the source table .txt file" )
	...
	while ~eof(lun) do begin
		readf, lun, line
		parts = strsplit( line, string(9b), /EXTRACT, COUNT=nParts )
		if nParts ne 9 then continue
		s = { $
		point:		'', $
		protocol:	'', $
		tracer:		'', $
		...
		ss:			0, $
		desc:		''}
		s.point = strtrim( parts[0], 2 )
		s.protocol = strtrim( parts[1], 2 )
		s.tracer = strtrim( parts[2], 2 )
		s.a0 = float( parts[3] )
		s.t1 = fix( parts[4] )
		s.t2 = fix( parts[5] )
		s.na = fix( parts[6] )
		...
		shielding_guiAddProtocol, pInfo, SOURCES=s

	endwhile
	close, lun
	free_lun, lun

end ; of shielding_guiOpenSources
