...
pro shielding_guiUpdateMeasurements, pInfo
	...
	for iROI=0, nROIs-1 do begin
		...
		strCoordsInM = format_float( data, DEC=2, /STRING )
		prefix = (strsplit( name, '_', /EXTRACT ))[0]
...
pro shielding_guiPrintLength, pInfo, oROI
	...
	length = format_float( length, DECIMAL=2, /STRING )
...
@'shielding_gui_shield_specs'
	...
	; Columns 1 & 3: xcoords
		...
		strCoord = number_formatter( float(entry), DECIMAL=2 )
...
function shielding_guiScrn2ImgPix, $
	...
	; Round to 2 decimal places (to the nearest cm)
	strCoords[0] = number_formatter( imgCoords[0], DECIMALS=2 )
	strCoords[1] = number_formatter( imgCoords[1], DECIMALS=2 )

	imgCoords = float( strCoords ) / fpScale
...
pro shielding_guiFindMax, $
	...
	strCoordsInM = string( maxCoords*fpScale)
	strCoordsInM[0] = number_formatter( maxCoords[0], DECIMAL=2 )
	strCoordsInM[1] = number_formatter( maxCoords[1], DECIMAL=2 )
	fltCoordsInM = float( strCoords )
	maxCoords = fltCoordsInM / fpScale
...
pro shielding_guiExtendTrimStop, pInfo, oEndROI
	...
	fmtCoords = strarr(2)
	fmtCoords[0] = number_formatter( coords[0]*scale, DECIMAL=2 )
	fmtCoords[1] = number_formatter( coords[1]*scale, DECIMAL=2 )
	coords = float(fmtCoords)/scale
